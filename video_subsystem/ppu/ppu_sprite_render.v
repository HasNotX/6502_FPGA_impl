 ...
 * Corrected OAM Bus priority locking and introduced 8x16 mode logic.
 */
...
module ppu_sprite_render (
    ...
    wire       is_8x16    = ppu_ctrl_reg[5];
    wire [8:0] spr_height = is_8x16 ? 9'd16 : 9'd8;
    ...
    // THE FIX: Lock the OAM bus across the entire evaluation matrix
    assign is_fetching = (eval_state != S_IDLE);

    // Combinatorial 8x16 CHR Address Generator
    wire [7:0] cur_spr_tile = eval_spr_tile[fetch_idx];
    wire [7:0] cur_spr_attr = eval_spr_attr[fetch_idx];
    wire [7:0] cur_spr_diff = eval_spr_y_diff[fetch_idx];
    
    reg [14:0] fetch_chr_addr;
    always @(*) begin
        reg [7:0] fetch_tile;
        reg [14:0] spr_base;
        reg [3:0] y_offset;
        ...
        if (is_8x16) begin
            spr_base   = {2'b00, cur_spr_tile[0], 12'd0};
            fetch_tile = {cur_spr_tile[7:1], 1'b0};
            y_offset   = cur_spr_diff[3:0]; // 0 to 15
            
            if (cur_spr_attr[7]) y_offset = 4'd15 - y_offset; // Vertical flip
            
            if (y_offset >= 4'd8) begin
                fetch_tile = fetch_tile + 8'd1;
                y_offset   = y_offset - 4'd8;
            end
        end else begin
            spr_base   = base_pat_addr;
            fetch_tile = cur_spr_tile;
            y_offset   = cur_spr_diff[3:0]; // safely use 4 bits
            
            if (cur_spr_attr[7]) y_offset = 4'd7 - y_offset; // Vertical flip
        end
        fetch_chr_addr = spr_base | ({7'd0, fetch_tile} << 4) | {12'd0, y_offset[2:0]};
    end
        ...
        if (reset) begin
            ...
            case (eval_state)
                ...
                S_EVAL_Y_CHK: begin
                    ...
                    // Hardware-accurate dynamic bounding box for 8x8 vs 8x16
                    if ({1'b0, next_nes_y} >= ({1'b0, oam_data} + 9'd1) && 
                        {1'b0, next_nes_y} <  ({1'b0, oam_data} + 9'd1 + spr_height) && 
                        oam_data < 8'd240 && 
