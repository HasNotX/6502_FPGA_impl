 ...
 * Implements a staggered 8-tick pipeline to perfectly absorb M10K BRAM latency.
 */
...
module ppu_bg_render (
    ...
    // -------------------------------------------------------------------------
    // 16-Bit Shift Registers (Continuous pixel shifting)
    // -------------------------------------------------------------------------
    ...
    // Tick 0 is now the LATCH cycle for Pattern Hi from the previous 8 ticks!
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (load_now) begin
                // Latch the final byte of the pipeline (Pattern Hi)
                pattern_hi_latch <= bg_mem_data;
                
                // Shift the old byte into the top half, load the fully assembled new tile into the bottom
                shift_pat_lo  <= {shift_pat_lo[14:7], pattern_lo_latch};
                shift_pat_hi  <= {shift_pat_hi[14:7], bg_mem_data}; // Use raw data to save a cycle
                shift_attr_lo <= {shift_attr_lo[14:7], {8{attr_latch[0]}}};
                shift_attr_hi <= {shift_attr_hi[14:7], {8{attr_latch[1]}}};
            end else begin
                // Standard 1-pixel shift
    ...
    // -------------------------------------------------------------------------
    // Output Multiplexer (Fine X Scroll)
    // -------------------------------------------------------------------------
