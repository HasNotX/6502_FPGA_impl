...
module cpu_fsm (
    ...
    output reg [15:0] PC,
    output reg [7:0] inst_reg, accum, X, Y, 
    output reg [7:0] s_pointer, s_reg,
        ...
        if (reset) begin
            ...
            case (state)
                ...
                S_OPERAND_LO: begin
                    operand_lo <= data_in;
                    PC         <= PC + 16'd1;
                    state      <= (extra_cycles_in >= 3) ? S_OPERAND_HI : S_EXECUTE;
                end
                ...
                S_EXECUTE: begin
                    if (addr_mode_in == MODE_IMMEDIATE) begin : exe_imm
                        ...
                        case (inst_reg)
                            ...
                            8'hE8, 8'hCA, 8'hC8, 8'h88, 8'hAA, 8'hA8, 8'h8A, 8'h98, 8'hBA: begin
                                case (inst_reg)
                                    8'hE8: src = X;
                                    8'hCA: src = X;
                                    8'hC8: src = Y;
                        ...
                        if ((alu_op_in == 4'd7  || alu_op_in == 4'd8  ||
                             ...
                             dest_reg_in == 2'd3)
                            state <= is_indirect_mode ? S_PTR_LO : S_RMW_READ;
                        else
                            state <= is_indirect_mode ? S_PTR_LO : S_WRITEBACK;
                    end
                end 
                ...
                S_WRITEBACK: begin : wb
                    ...
                    endcase
                    r3 = fr3[7:0];
                    if (alu_op_in == 4'd5) begin         
                        s_reg[7] <= data_in[7];
                        s_reg[6] <= data_in[6];
                        s_reg[1] <= ((accum & data_in) == 8'h00);
                    end else if (alu_op_in == 4'd6) begin  
                ...
                S_RTI_PULL_SR: begin
                    s_reg     <= (data_in | 8'h20) & 8'hEF;
                    s_pointer <= s_pointer + 8'd1;
                    state     <= S_RTI_PULL_PCL;
                end
                
                S_RTI_PULL_PCL: begin
                    operand_lo <= data_in;
                    s_pointer  <= s_pointer + 8'd1;
                    state      <= S_RTI_PULL_PCH;
                end
                ...
                S_RESET_VEC_LO: begin 
                    PC[7:0]  <= data_in; 
                    state <= S_RESET_VEC_HI; 
                end
                
                S_RESET_VEC_HI: begin 
                    PC[15:8] <= data_in; 
                    state <= S_FETCH; 
                end
    ...
    always @(*) begin
        case (state)
            ...
            S_JSR_JUMP:
                addr_sel = 3'd0;
            S_RTS_PULL_LO, S_RTS_PULL_HI, S_RTS_INC:
                addr_sel = 3'd5;
            S_RTI_PULL_SR, S_RTI_PULL_PCL, S_RTI_PULL_PCH:
                addr_sel = 3'd5;
            S_BRK_PUSH_PCH, S_BRK_PUSH_PCL, S_BRK_PUSH_SR:
                addr_sel = 3'd5;
            S_BRK_VEC_LO, S_BRK_VEC_HI:
                addr_sel = 3'd3;
            S_JMP_IND_LO:
                addr_sel = 3'd1;
            S_JMP_IND_HI:
                addr_sel = 3'd2;
            S_FETCH, S_OPERAND_LO, S_OPERAND_HI:
                addr_sel = 3'd0;
            S_WRITEBACK, S_PTR_LO, S_RMW_READ, S_RMW_WRITE:
                addr_sel = 3'd1;
            S_PTR_HI:
                addr_sel = 3'd2;
            S_INDIRECT_EXEC:
                addr_sel = 3'd4;
            S_RESET_VEC_LO, S_RESET_VEC_HI:
                addr_sel = 3'd3;
            S_PUSH_EXEC, S_PULL_EXEC:
                addr_sel = 3'd5;
            S_STORE_EXEC:
                addr_sel = 3'd1;
            S_STORE_IND_EXEC:
